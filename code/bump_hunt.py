...
def process(data_loader, data_len):
    ...
    t = tqdm.tqdm(enumerate(data_loader),total=data_len / batch_size)
    with torch.no_grad():
        for i, data in t:
            if(len(data) >= 2):
                jet1 = data[0]
...
def bump_hunt():
    ...
    bb1_loader = DataListLoader(bb1, batch_size=batch_size)
    ...
    bb2_loader = DataListLoader(bb2, batch_size=batch_size)
